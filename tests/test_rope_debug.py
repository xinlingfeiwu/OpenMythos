"""
Standalone RoPE debug script.

Kept under tests/ for convenience, but wrapped in `main()` so pytest import
does not execute the debug routine during collection.
"""
...
def main() -> None:
    ...
    print(
        f"  Recovery succeeded (atol=1e-5): {torch.allclose(x3, recovered, atol=1e-5)}"
    )
    ...
    out_buggy = apply_rope(decode_token, freqs[:1])
    out_fixed = apply_rope(decode_token, freqs[prompt_len : prompt_len + 1])
